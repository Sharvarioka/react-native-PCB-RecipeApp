...
const HomeScreen = () => {
  ...
  const onStopRecord = async () => {
    ...
    const getFinalEditDistanceResult = async (userString) => {
      ...
      for (let i = 0; i < responseJsonFile.response.length; i++) {
        res1 = await editDistance(
          userString,
          responseJsonFile.response[i].Actions,
          i
        );
        if (res1) {
    ...
    console.log("responseJSon.text is", responseJson.text);
    const foundArrayOfObjects = await getFinalEditDistanceResult(
      "सोल्डर की नोक"
    );
    ...
    if (foundArrayOfObjects.length) {
      ...
      ToastAndroid.show(
        "Please provide a correct annotation",
        ToastAndroid.LONG
      );
    }
  };
  ...
  const onPressButton = (ele) => {
    ...
    const total_sec =
      parseFloat(start_timestamp[0]) * 3600 +
      parseFloat(start_timestamp[1]) * 60 +
      parseFloat(start_timestamp[2]);
    ...
    <View>
      <View style={optionsShow ? styles.renderLogic : ""}>
        <Video
          ...
          source={require("../How_to_solder.mp4")}
      ...
      <ScrollView style={optionsShow ? "" : styles.renderLogic}>
        ...
        <TouchableOpacity
          ...
          onPress={() => {
            setOptionsShow(false), (arrayOfObjects = []);
          }}
        >
          <Text>Exit</Text>
        </TouchableOpacity>
